...
export async function POST(request: NextRequest) {
  try {
    const { image, tags, price, pickup_location } = await request.json() as { image: string; tags: Tags; price?: number | null; pickup_location?: string};
    ...
    if (pickup_location != null) contextParts.push(`pickup_location=${pickup_location}`);
    if (price !== undefined && price !== null) contextParts.push(`price=${price}`);
